...
export function renderAuthUI(){
  ...
  if(user){
    el.innerHTML = `
      <span class="iconbtn" title="Account">${escapeHtml(user.name)}</span>
      <button class="iconbtn" id="logoutBtn" title="Logout">Logout</button>
    `;
    document.getElementById("logoutBtn")?.addEventListener("click", logout);
  }else{
    el.innerHTML = `
      <a class="iconbtn" href="/login.html" title="Login">Login</a>
      <a class="iconbtn" href="/register.html" title="Register">Register</a>
    `;
  }
}
